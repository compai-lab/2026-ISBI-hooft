...
Description:
This script trains deep learning models for multi-organ segmentation on volumetric medical data (e.g., AMOS-BPR).
It supports various architectural strategies such as baseline UNet and LocBAM. Training includes advanced data
transforms, sliding window inference, mixed-precision, polynomial LR scheduling, and optional spatial coordinate augmentation.

Key features:
- Supports 3D patch-based training with class-balanced sampling and dynamic patch sizing.
- Integrates coordinate channels and body part regression (BPR) as additional model input.
- Uses `ConcatChannelsd` and `SplitChannelsd` for modular and memory-efficient input handling.
- Uses `OptimizeCoordinateChannels` and `ReconstructCoordinateChannels` to compress and reconstruct spatial coordinate maps on the fly.
- `BodyPartRegressorScoreTransform` rescales and linearly extrapolates BPR scores to preserve spatial continuity across the volume.

Note:
These custom transforms help significantly reduce memory overhead and I/O load during preprocessing,
while preserving all spatial and anatomical context needed for effective model learning.
"""
...
class OptimizeCoordinateChannels(MapTransform):
    ...
    def __call__(self, data):
        ...
        for key in self.keys:
            ...
            print(coord_x[0], flush=True)
            print(coord_x[-1], flush=True)
            print(" ", flush=True)

            print(coord_y[0], flush=True)
            print(coord_y[-1], flush=True)
            print(" ", flush=True)

            print(coord_z[0], flush=True)
            print(coord_z[-1], flush=True)
            print(" ", flush=True)
...
if __name__ == "__main__":
    ...
    def build_dataset(current_patch_size, sample_batch_size, crop_batch_size, fold_idx, sampling):
        ...
        cache_dir = "/vol/ciamspace/datasets/amos_bpr/cache" # <-- CHANGE THIS TO YOUR LOCAL DATA LOCATION
        if False:
    ...
    def validation(epoch_iterator_val, patch_size, batch_size, sampling, max_samples=None):
        ...
        with torch.no_grad():
            for batch in epoch_iterator_val:
                val_inputs, val_labels = (batch["image"].cuda(non_blocking=True), batch["label"].cuda(non_blocking=True))
            ...
            #for fold_idx in range(5):
                ...
                while global_step <= max_iterations:
                    ...
                    for step, batch in enumerate(epoch_iterator):
                        ...
                        x, y = (batch["image"].cuda(non_blocking=True), batch["label"].cuda(non_blocking=True))
                        ...
                        #     x[:, 1:2, :, :, :] = (((x[:, 1:2, :, :, :] * (250 - (-175)) + (-175) )/70 )+0.050)
                        #     x[:, 1:2, :, :, :] = torch.clamp(x[:, 1:2, :, :, :], min=0.000 , max=1.000)
                        #print(x.shape)
                ...
                torch.save(model.state_dict(), os.path.join(logdir, "iso_AMOS22_UNet_final_" + str(LocContext)+"_"+str(p_size)+"_"+str(fold_idx)  + "_" + str(performance_string) +".pth"))