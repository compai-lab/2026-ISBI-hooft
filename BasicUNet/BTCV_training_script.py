...
Description:
This script trains deep learning models for multi-organ segmentation on volumetric medical data (e.g., BTCV).
For both BasicUNet and BasicUNetLocBAM
"""
...
from LocBAM.LocBAM_1D import BasicUnetLocBAMs2  # check the directory
...
if __name__ == "__main__":
    # Optional: Use parser arguements to define which conditions/folds/patch_size you want to run
    ...
    def build_dataset(current_patch_size, sample_batch_size, crop_batch_size, fold_idx, sampling):
        ...
        data_dir = "/vol/ciamspace/datasets/btcv/"  # <-- CHANGE THIS TO YOUR LOCAL DATA LOCATION

        
        split_json = "dataset_" + str(fold_idx) + ".json"   #ensure 5 .json files with respective datasplits 20/80 are included
    ...
    for LocContext in ["Baseline", "CoordConv", "LocBAM"]: #,#adjust for conditions
        for p_size in [ "small", "large" ]:# ,, "small"
            for fold_idx in range(5):
                if LocContext == "Baseline":
                    sampling = "Baseline"
                    nr_in_channels = 1
                elif LocContext == "CoordConv":
                    sampling = "CoordConv"
                    nr_in_channels = 4
                ...
                # optimizer = torch.optim.Adam(model.parameters(), lr=new_lr, weight_decay=3e-5)   #alternative optimizer
                ...
                writer.flush()
                writer.close()
                torch.save(model.state_dict(), os.path.join(logdir, "UNet_final_" + str(LocContext)+"_"+str(p_size)+"_"+str(fold_idx) +".pth"))